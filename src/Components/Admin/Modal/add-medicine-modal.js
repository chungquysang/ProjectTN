...
import { addMedicine, uploadAvatarMedicine } from "../../../services/API/Medicine";

export default function AddMedcine({ onClose, onSuccess }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setDosageForms(dosageRes || []);
      } catch (error) {
        console.error("Lỗi khi lấy danh mục hoặc dạng bào chế:", error);
      }
    }
    fetchData();
  }, []);
  ...
  const validateForm = () => {
    ...
    if (!formData.dosageForm)
      newErrors.dosageForm = "Vui lòng chọn dạng bào chế";
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = [
        {
          ...
          medicineStatus: formData.status,
          medicinePrice: formData.price,
          medicineCategory_id: formData.category,
          dosage_form_id: formData.dosageForm,
          medicineAvatar: "",
        }
      ];
      ...
      if (formData.image && medicineId) {
        await uploadMedicineImage(medicineId, formData.image);
      }
                  ...
                  <div className="col-span-1 md:col-span-1 space-y-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {categories.map((category) => (
                          <option key={category.medicineCategoryId} value={category.medicineCategoryId}>
                    ...
                    <div>
                      ...
                      <select
                        id="dosageForm"
                        name="dosageForm"
                        className={`w-full px-3 py-2 border ${errors.dosageForm
                          ? "border-red-500"
                          : "border-gray-300"
                          } rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500`}
