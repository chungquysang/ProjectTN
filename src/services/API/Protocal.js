...
const BASE_URL = "http://localhost:8888/api/v1/protocols";

// Truyền doctorId trực tiếp từ nơi gọi hàm
export const GetinformationLT = async (id_user) => {
...
export const updateProtocolInformation = async (id_user, updatedData,) => {
    const token = localStorage.getItem("token");
...
export const loadAvatarProtocol = async (userId) => {
    const token = localStorage.getItem("token");
    if (!token) {
        throw new Error("Không tìm thấy token.");
    }
...
export const uploadAvatarProtocol = async (userId, formData) => {
    const token = localStorage.getItem("token");
    if (!token) {
        throw new Error("Không tìm thấy token.");
    }
